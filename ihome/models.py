...
class User(BaseModel, db.Model):
    ...
    def info_to_dict(self):
        ...
        user_dict = {
            ...
            "name": self.name,
            'mobile': self.mobile,
            'avatar': IMAGE_URL+self.avatar_url if self.avatar_url else '',
...
class House(BaseModel, db.Model):
    ...
    def to_basic_dict(self):
        ...
        house_dict = {
            ...
            "user_avatar":  IMAGE_URL+self.user.avatar_url if self.user.avatar_url else "",
    ...
    def to_full_dict(self):
        ...
        orders = Order.query.filter(Order.house_id == self.id, Order.status == "COMPLETE", Order.comment != None) \
            .order_by(Order.update_time.desc()).limit(5)
        for order in orders:
            comment = {
