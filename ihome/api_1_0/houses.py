...
import json
from constants import AREA_TIME
...
def get_area_info():
    ...
    # 因为要传入前端以json格式，只有dict能转换成json，所以将对象转换成字典
    area_dict_li = []
    for area in area_li:
        d = {
            "aid": area.id,
    ...
    # 将数据转换成json格式
    resp_dict = dict(errno=RET.OK, data=area_dict_li)
    resp_json = json.dumps(resp_dict)
    redis_store.setex('area_info', AREA_TIME, resp_json)
...
def save_house_image():
    ...
    if not house:
        return jsonify(errno=RET.DATAERR, errmsg='房屋不存在，请先创建房屋信息')
    ...
    # 保存图片到数据库中
    house_image = HouseImage(house_id=house_id, url=url)
...
def get_house_index():
    ...
    # 从缓存中尝试获取数据
    try:
        rets = redis_store.get("home_page_data")
        ret = str(rets, encoding="utf-8")
    except Exception as e:
        pass
    ...
    if rets:
        ...
        try:
            ...
            houses = House.query.order_by(House.order_count.desc()).limit(3)
        except Exception as e:
        ...
        try:
            redis_store.setex("home_page_data", 3600, json_houses)
        except Exception as e:
            pass

        return '{"errno":0, "errmsg":"OK", "data":%s}' % json_houses, 200, {"Content-Type": "application/json;charset=utf-8"}
...
def get_house_detail(house_id):
    ...
    # 先从redis缓存中获取信息
    try:
        rets = redis_store.get("house_info_%s" % house_id)
        ret = str(rets, encoding="utf-8")
    except Exception as e:
        print('获取失败')
    if rets:
    ...
    try:
        redis_store.setex("house_info_%s" % house_id, 600, json_house)
    except Exception as e:
        pass
...
# 获取的路径 GET  /api/v1.0/houses?sd=2020-04-05&ed=2020-04-06&aid=10&sk=new&p=1
@api.route('/houses')
def get_house_list():
    """获取房屋的列表信息（搜索页面）"""
    # 先获取url后边的参数
    ...
    # 查询数据库
    # 补充排序条件
    if sort_key == "booking":  # 入住做多   filter_param里边是多个查询语句，加上*号是将其拆分为一个一个的
